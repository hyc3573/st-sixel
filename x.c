...
{
    xdrawrect(&dc.col[IS_SET(MODE_REVERSE) ? defaultfg : defaultbg], x1, y1,
              x2 - x1, y2 - y1);
}
...
int xloadfont(Font *f, FcPattern *pattern)
{
    ...
    f->match.cairo =
        cairo_ft_font_face_create_for_ft_face(f->match.face, FT_LOAD_COLOR);

    if ((FcPatternGetInteger(pattern, FC_SLANT, 0, &wantattr) == FcResultMatch))
    {
        /*
         ...
         * Try to mitigate.
         */
        if ((FcPatternGetInteger(f->match.pattern, FC_SLANT, 0, &haveattr) !=
             FcResultMatch) ||
            haveattr < wantattr)
        {
            f->badslant = 1;
    ...
    if ((FcPatternGetInteger(pattern, FC_WEIGHT, 0, &wantattr) ==
         FcResultMatch))
    {
        if ((FcPatternGetInteger(f->match.pattern, FC_WEIGHT, 0, &haveattr) !=
             FcResultMatch) ||
            haveattr != wantattr)
        {
            f->badweight = 1;
...
void xunloadmatchfont(MatchFont match)
{
...
int ximopen(Display *dpy)
{
    ...
    xw.ime.xic = XCreateIC(xw.ime.xim, XNInputStyle, ximstyle, XNClientWindow,
                           xw.win, XNFocusWindow, xw.win, XNDestroyCallback,
                           &icdestroy, attrs ? XNPreeditAttributes : NULL,
                           attrs ? attrs->list : NULL, NULL);
    if (attrs)
...
XIMStyle ximpickupstyle(XIM xim)
{
...
XIMPreeditAttrs *ximcreatepreeditattrs(Display *dpy, XIMStyle ximstyle)
{
    char pat[32];
    ...
    if (XIMPreeditPosition & ximstyle)
    {
        ...
        attrs->list =
            XVaCreateNestedList(0, XNFontSet, attrs->fontset, XNSpotLocation,
                                &spot, XNForeground, dc.col[defaultfg].pixel,
                                XNBackground, dc.col[defaultbg].pixel, NULL);
        return attrs;
    }
...
void ximdestroypreeditattrs(Display *dpy, XIMPreeditAttrs *attrs)
{
    XFree(attrs->list);
...
FT_UInt xselectfont(Rune rune, int frcflags, MatchFont **ret)
{
    FT_UInt glyphidx;
    Font *font = &dc.font;
...
int xmakeglyphfontspecs(GlyphFontSpec *specs, const Glyph *glyphs, int len,
                        int x, int y)
{
...
void xdrawglyphfontspecs(const GlyphFontSpec *specs, Glyph base, int len, int x,
                         int y)
{
...
void xdrawglyphfontspec(XColor *fg, const GlyphFontSpec *specs, int len)
{
    cairo_font_face_t *prev = NULL;
    int i;

    cairo_set_font_size(xw.draw, defaultfontsize);
    cairo_set_source_rgb(xw.draw, CONVCOL(fg->red), CONVCOL(fg->green),
                         CONVCOL(fg->blue));
    for (i = 0; i < len; i++)
    {
...
void xdrawrect(XColor *col, int x, int y, int width, int height)
{
...
void xdrawcursor(int cx, int cy, Glyph g, int ox, int oy, Glyph og)
{
    ...
    if (IS_SET(MODE_FOCUSED))
    {
        ...
        case 4: /* Steady Underline */
            xdrawrect(&drawcol, win.hborderpx + cx * win.cw,
    ...
    else
    {
        ...
        xdrawrect(&drawcol, win.hborderpx + (cx + 1) * win.cw - 1,
                  win.vborderpx + cy * win.ch, 1, win.ch - 1);
        xdrawrect(&drawcol, win.hborderpx + cx * win.cw,
...
int xsixelinit(SixelContext *ctx)
{
    return sixel_parser_init(&ctx->state, 0,
                             sixelbg /* dc.col[defaultbg].pixel */, 1, win.cw,
                             win.ch);
}

int xsixelparse(SixelContext *ctx, unsigned char *u, int len)
{
    return sixel_parser_parse(&ctx->state, u, len);
}

void xsixelnewimage(SixelContext *ctx, int tx, int ty)
{
    ImageList *new_image;
    int i;
...
void xsixeldeleteimage(SixelContext *ctx, ImageList *im)
{
    if (im->prev)
...
void xsixelscrolldown(SixelContext *ctx, int n, int bottom)
{
    ImageList *im;
...
void xsixelscrollup(SixelContext *ctx, int n, int top)
{
    ImageList *im;
    for (im = ctx->images; im; im = im->next)
    {
...
void xdrawsixel(SixelContext *ctx, Line *line, int row, int col)
{
    ImageList *im, *tmp;
    int x, y;
    int n = 0;
    ...
    for (im = ctx->images; im;)
    {
        ...
        {
            im->pixmap =
                (void *)XCreatePixmap(xw.dpy, xw.win, im->width, im->height,
                                      DefaultDepth(xw.dpy, xw.scr));
        ...
        n = 0;
        for (y = im->y;
             y < (im->y + (im->height + win.ch - 1) / win.ch) && y < row; y++)
        {
            if (y >= 0)
            {
                for (x = im->x;
                     x < (im->x + (im->width + win.cw - 1) / win.cw) && x < col;
                     x++)
                {
                    if (!rects)
                    ...
                    {
                        if (n > 0 &&
                            rects[n - 1].x + rects[n - 1].width ==
                                borderpx + x * win.cw &&
                            rects[n - 1].y == borderpx + y * win.ch)
                        {
                        ...
                        else
                        {
                            ...
                            if (++n == nlimit &&
                                (rects = realloc(rects, sizeof(XRectangle) *
                                                            (nlimit *= 2))) ==
                                    NULL)
                                die("Out of memory\n");
                        }
...
int resourceload(XrmDatabase db, char *name, enum resource_type rtype,
                 void *const dst)
{
    char **sdst = dst;
    int *idst = dst;
